...
inline Array2d<T>::Array2d() 
	: nrows{0}, ncols{0}, size{0}, storage{ROWMAJOR}, elems{nullptr}
{
}

// Full-arg constructor
template <typename T>
inline Array2d<T>::Array2d(int nr, int nc, MStype st)
	: nrows{nr}, ncols{nc}, size{nr*nc}, storage{st}, elems{new T[nr*nc]}
{
	if(nc==0)
	{
	...
	if(nr==0)
	{
		std::cout << "Error: Number of rows is zero.\n";
	}
...
inline Array2d<T>::Array2d(const Array2d<T>& other)
	: nrows{other.nrows}, ncols{other.ncols}, size{other.nrows*other.ncols}, storage{other.storage}, 
	elems{new T[other.nrows*other.ncols]}
{
...
//Separate setup function in case no-arg constructor has to be used
template <typename T>
inline void Array2d<T>::setup(int nr, int nc, MStype st)
{
	if(nc==0)
	{
...
template <typename T>
inline void Array2d<T>::setupraw(int nr, int nc, MStype st)
{
	//std::cout << "\nEntered setupraw";
	if(nc==0)
	{
...
template <typename T>
inline void Array2d<T>::identity()
{
	T one = (T)(1);
	T zero = (T)(0);
...
// function to set matrix elements from a ROW-MAJOR std::vector
template <typename T>
inline void Array2d<T>::setdata(const T* A, int sz)
{
#ifdef DEBUG
	if(sz != size)
	{
...
template <typename T>
inline void Array2d<T>::set(int i, int j, T data)
{
#ifdef DEBUG
...
template <typename T>
inline MStype Array2d<T>::storetype() const { return storage;}

template <typename T>
inline void Array2d<T>::mprint() const
{
	std::cout << "\n";
	if(storage==COLMAJOR)
		for(int i = 0; i < nrows; i++)
...
template <typename T>
inline void Array2d<T>::fprint(std::ofstream& outfile) const
{
	//outfile << '\n';
...
template <typename T>
inline void Array2d<T>::fread(std::ifstream& infile)
{
...
template <typename T>
inline T Array2d<T>::maxincol(int j) const
{
	T max = get(0,j);
	for(int i = 0; i < nrows; i++)
...
template <typename T>
inline T Array2d<T>::maxinrow(int i) const
{
	T max = get(i,0);
...
template <typename T>
inline T Array2d<T>::minincol(int j) const
{
	T min = get(0,j);
	for(int i = 0; i < nrows; i++)
...
template <typename T>
inline T Array2d<T>::mininrow(int i) const
{
	T max = get(i,0);
	for(int j = 0; j < nrows; j++)
		if(max > get(i,j)) max = get(i,j);
...
template <typename T>
inline T Array2d<T>::average() const
{
	T avg = 0;
	for(int i = 0; i < size; i++)
		avg += elems[i];
...
template <typename T>
inline T Array2d<T>::l2norm() const		// sums the square of all elements in the matrix and returns the square root of this sum
{
	T tot = 0;
	for(int i = 0; i < size; i++)
	{
...
// function to return a sub-matrix of this matrix
template <typename T>
inline Array2d<T> Array2d<T>::sub(int startr, int startc, int offr, int offc) const
{
...
//Function that returns a given column of the matrix as a row-major matrix
template <typename T>
inline Array2d<T> Array2d<T>::col(int j) const
{
	Array2d<T> b(nrows, 1);
	if(storage==COLMAJOR)
		for(int i = 0; i < nrows; i++)
...
template <typename T>
inline Array2d<T> Array2d<T>::row(int i) const
{
	Array2d<T> b(1, ncols);
	if(storage==COLMAJOR)
		for(int j = 0; j < ncols; j++)
...
template <typename T>
inline void Array2d<T>::replacecol(int j, Array2d<T> b)
{
#ifdef DEBUG
...
//Function for replacing a row
template <typename T>
inline void Array2d<T>::replacerow(int i, Array2d<T> b)
{
#ifdef DEBUG
...
//transpose
template <typename T>
inline Array2d<T> Array2d<T>::trans() const
{
...
// Multiply a matrix by a scalar. Note: only expressions of type A*3 work, not 3*A
template <typename T>
inline Array2d<T> Array2d<T>::operator*(T num)
{
...
/**	The matrix addition and subtraction operators are inefficient! Do not use in long loops. */
template <typename T>
inline Array2d<T> Array2d<T>::operator+(Array2d<T> B) const
{
#ifdef DEBUG
...
template <typename T>
inline T Array2d<T>::dot_product(const Array2d<T>& A)
/* Returns sum of products of respective elements of flattened arrays containing matrix elements of this and A */
{
	T* elemsA = A.elems;
	#ifdef _OPENMP
